...
Note that $\AGL_1(\mathbb{R}) \subset \PGL_2(\mathbb{R})$ by using $c=0$ and $d = 1$ in the definition above. Moreover, every element of $\AGL_1(\mathbb{R})$ fixes $\infty$, and in fact, $\AGL_1(\mathbb{R})$ is equal to the stabilizer of $\infty$ in $\PGL_2(\mathbb{R})$. Also note that the function $i(x) = \frac{1}{x} \in \PGL_2(\mathbb{R})$, where $i(0) = \infty$ and $i(\infty) = 0$. If we scale the circle above so that $-1$ and $1$ are the endpoints of the horizontal diameter, then the function $i(x)$ can be viewed as a reflection over that diameter.
