...
We now work to show that $\PGL_2(\mathbb{R})$ acts $3$-transitively on $\Proj^1(\mathbb{R})$. Set $G = \PGL_2(\mathbb{R})$. Let's first prove that the action is transitive.  We'll do this by showing that every $r\in \Proj^1(\mathbb{R})$ is contained in $\Orb_G(\infty)$. If $r=\infty$, then 
