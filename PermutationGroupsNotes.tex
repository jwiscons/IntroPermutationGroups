...
\begin{problem*}
Starting from the usual action of $\Sym(n)$ on $X=\{1,\ldots,n\}$, we can consider several related actions. Let $\Sub_m(X)$ be the collection of all subsets of $X$ that have cardinality $m$, and let $\Part_{k\times \ell}(X)$ denote the set of all partitions of $X$ into $k$ parts of equal size $\ell$ (so here we must have $n=k\ell$).

For example, $\Sub_2(\{1,2,3,4\}) = \{ \{1,2\}, \{1,3\}, \{1,4\}, \{2,3\}, \{2,4\}, \{3,4\}\}$.
\end{problem*}
