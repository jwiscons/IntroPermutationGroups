...
This section will introduce the notion of \emph{relational complexity} for a permutation group. This is a relatively new topic of study that has its roots in an area of mathematical logic known as model theory. Quite of bit of research has occurred lately around relational complexity, and there are many questions that remain.

The definition might be challenging to wrap one's head around at first, but we will spend some time unpacking what it means. However, the (very important) question of why one would consider this definition in the first place is best answered in model-theoretic terms, but we will simply refer out for that: see for example~\cite[Section~2]{ChG00} or \cite[Section~2.2]{WiJ16b}. Sorry!
...
\item The \textbf{relational complexity} of the action, denoted $\rc(G,X)$, is the smallest $r$ such that $\bar{x}\sim_r\bar{y}$ implies $\bar{x}\sim_k\bar{y}$ of  for all $k\ge r$ and all $\bar{x},\bar{y} \in X^k$.
\end{enumerate}
\end{definition*}
...
\end{tikzpicture}
\end{center}
However, if the tuples are $3$-equivalent, then there would exist a single $g\in G$ that simultaneously maps all three elements of $\bar{x}$ to the corresponding elements of $\bar{y}$, i.e.  $g\cdot(x_1,x_2,x_3) = (y_1,y_2,y_3)$. 
\begin{center}
\begin{tikzpicture}[line width = 1.2]
\footnotesize
\node at (0,1.5) {$(x_1,x_2,x_3)$};
